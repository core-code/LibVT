...
const u8 OriginMask = 0x30;

#pragma pack(1)
typedef struct
{
	u8	bIDLength;			// Length of the ID field
	u8	bCMapType;			// 0/1
	u8	bImgType;			// 2 = uncompressed true color
	u16	CMapStart;			// Index of the first palette entry
	u16	CMapLength;			// Length of the color map
	u8	CMapBPP;			// Bits per pixel in the color map

	u16	XOrigin, YOrigin;
	u16	Width, Height;		// Image size
	u8	bPlanes;			// Bits per pixel
	u8	bImgDescription;	// Origin / Alpha channel bits
} TGAHeader;
#pragma pack()
